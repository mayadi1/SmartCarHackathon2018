...
const get_vehicles = function(access_token, callback) {
  ...
  .then(function(res) {
    if (!res) {
      ...
      if (res.vehicles) {
        res.vehicles.forEach(function(vehicle_id) {
          get_vehicle(access_token, vehicle_id).info().then(function(data) {
            vehicles.push(data); 
          });
        });
        callback(vehicles);
...
var credentials = JSON.parse('{"accessToken":"ee3674b0-38e3-479b-8a42-00d9430d3b79","refreshToken":"186c09c5-ae41-48dd-9905-20ad819ff555","expiration":"2018-09-23T04:29:48.580Z","refreshExpiration":"2018-11-22T02:29:48.580Z"}');
...
const get_credentials = function(callback) {
  ...
  if (dates.compare(access_token_expiration_dt, curr_dt) < 0) {
    ...
    renew_access(credentials.refreshToken, callback);
  } else {
